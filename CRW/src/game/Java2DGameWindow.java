...
public class Java2DGameWindow extends Canvas {
	...
	public void startRendering()
	{
		...
		panel.setPreferredSize(new Dimension((arenaSize*2) + 600, arenaSize*2)); //600 for scoreboard
		panel.setLayout(null);

		// setup our canvas size and put it into the content of the frame

		setBounds(0, 0, width*2 + 600, height*2); //600 for scoreboard
		panel.add(this);
	...
	protected void drawScore(Graphics2D g, Stack<Robot> robots)
	{
		...
		for(int i = 0; i < robots.size(); i ++)
		{
			if(i == 0 || i == 1){
				...
				g.drawString(name.toUpperCase(), 10 + 125 * i, 22);
				g.drawString("Energy:" + rob.energy, 10 + 125 * i, 40);
				g.drawString("Active:" + rob.alive, 10 + 125 * i, 55);
				g.drawString("Hull:" + rob.hull, 10 + 125 * i, 70);
				g.drawString("Shield:" + rob.shield, 10 + 125 * i, 85);
			}
			else if(i == 2 || i == 3)
			{
				Robot rob = robots.get(i);
				String name = rob.name.substring(0,rob.name.indexOf('.'));
				g.drawString(name.toUpperCase(), 10 + 125 * ((i-1)/2), 112);
				g.drawString("Energy:" + rob.energy, 10 + 125 * ((i-1)/2), 130);
				g.drawString("Active:" + rob.alive, 10 + 125 * ((i-1)/2), 145);
				g.drawString("Hull:" + rob.hull, 10 + 125 * ((i-1)/2), 160);
				g.drawString("Shield:" + rob.shield, 10 + 125 * ((i-1)/2), 175);
			}
		}
		g.drawString("Winner:" + winner, 10, 200);
		g.drawString("Error:" + error, 10, 225);
	...
	protected void preGameBoard(Graphics2D g)
	{
		g.setColor(Color.BLACK);
		g.setFont(new Font("Courier New", Font.BOLD, 15));
		...
		if(f1 != null)
		{
			String name = f1.getName().substring(0,f1.getName().indexOf('.'));
			g.drawString("1: " + name.toUpperCase(), 10 + 125 * 0, 22);
		}
		if(f2 != null)
		{
			String name = f2.getName().substring(0,f2.getName().indexOf('.'));
			g.drawString("2: " + name.toUpperCase(), 10 + 125 * 1, 22);
		}
		if(f3 != null)
		{
			String name = f3.getName().substring(0,f3.getName().indexOf('.'));
			g.drawString("3: " + name.toUpperCase(), 10 + 125 * 0, 112);
		}
		if(f4 != null)
		{
			String name = f4.getName().substring(0,f4.getName().indexOf('.'));
			g.drawString("4: " + name.toUpperCase(), 10 + 125 * 1, 112);
		}
	}
	...
	void preGameLoop()
	{
		while (gameRunning)
		{
			...
			g.fillRect(0, 0, arenaSize + 600, arenaSize);
			
			getDrawGraphics().translate(arenaSize, 0);
			preGameBoard(getDrawGraphics());
			getDrawGraphics().translate(-arenaSize, 0);
			getDrawGraphics().drawLine(arenaSize+5, 0, arenaSize+5, arenaSize);
